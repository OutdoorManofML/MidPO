...
class CostPreferenceSample(TypedDict, total=True):
    better_input_ids: torch.LongTensor  # size = (L,)
    worse_input_ids: torch.LongTensor  # size = (L,)


class CostPreferenceBatch(TypedDict, total=True):
...
class CostPreferenceDataset(TokenizedDataset):
    def preprocess(self, raw_sample: RawSample) -> CostPreferenceSample:
    ...
    def get_collator(self) -> Callable[[list[dict[str, torch.Tensor]]], dict[str, torch.Tensor]]:
        return CostPreferenceCollator(self.tokenizer.pad_token_id)


class CostPreferenceCollator(CollatorBase):
    def __call__(self, samples: list[CostPreferenceSample]) -> CostPreferenceBatch:
