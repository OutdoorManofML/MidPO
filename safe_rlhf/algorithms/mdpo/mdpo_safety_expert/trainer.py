...
from safe_rlhf.datasets import SafetyExpertPreferenceDataset
...
class SafetyExpertTrainer(SupervisedTrainer):
    TRAINING_TYPE = 'mdpo_safety_expert'

    DATASET_TYPE = SafetyExpertPreferenceDataset
    ...
    def init_models(self) -> None:
        ...
        self.safety_reward_model = AutoModelForScore.from_pretrained(
            '/root/safe-rlhf/dataroot/models/safety-reward-eval', torch_dtype=torch.bfloat16, device_map='auto')
        for param in self.safety_reward_model.parameters():
            param.requires_grad = False

        rm_tokenizer = AutoTokenizer.from_pretrained('/root/safe-rlhf/dataroot/models/safety-reward-eval')
    ...
    def loss(  # pylint: disable=too-many-locals
        ...
        with torch.no_grad():
            ...
            # New algorithm start
            reward_sequence_loss = self.safety_reward_model(
                input_ids=torch.cat([better_input_ids, worse_input_ids], dim=0),
    ...
    def train_step(
        ...
        better_input_ids: torch.LongTensor,  # size = (B, L)
        better_attention_mask: torch.BoolTensor,  # size = (B, L)
        better_is_unsafe: torch.BoolTensor,
        worse_input_ids: torch.LongTensor,  # size = (B, L)
        worse_attention_mask: torch.BoolTensor,  # size = (B, L)
        worse_is_unsafe: torch.BoolTensor,
    ) -> dict[str, Any]:
